...
export const calculateTotalPrice = (basePrice: number, participants: number, discountPercentage = 0): number => {
...
export const isExperienceAvailable = (experience: any, date: Date): boolean => {
  ...
  return experience.available.includes('All seasons') || 
...
export const validateExperienceData = (data: any): { isValid: boolean; errors: string[] } => {
  const errors: string[] = []
