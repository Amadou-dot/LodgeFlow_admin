...
export default function BookingForm({ onSuccess, onCancel }: BookingFormProps) {
  ...
  useEffect(() => {
    ...
    const discountedPrice = selectedCabin.discount > 0 
    ...
    const parkingFee = formData.hasParking && !settings.parkingIncluded 
      ? settings.parkingFee * numNights 
      : 0;
    
    const earlyCheckInFee = formData.hasEarlyCheckIn ? settings.earlyCheckInFee : 0;
    const lateCheckOutFee = formData.hasLateCheckOut ? settings.lateCheckOutFee : 0;
    
    const extrasPrice = breakfastPrice + petFee + parkingFee + earlyCheckInFee + lateCheckOutFee;
  ...
  }, [selectedCabin, settings, numNights, formData.hasBreakfast, formData.hasPets, formData.hasParking, formData.hasEarlyCheckIn, formData.hasLateCheckOut, formData.numGuests]);
  ...
  const validateForm = (): string[] => {
    ...
    if (formData.numGuests < 1) errors.push("Number of guests must be at least 1");
    if (selectedCabin && formData.numGuests > selectedCabin.capacity) {
      errors.push(`Number of guests cannot exceed cabin capacity (${selectedCabin.capacity})`);
    }
    if (settings && numNights < settings.minBookingLength) {
      errors.push(`Minimum booking length is ${settings.minBookingLength} nights`);
    }
    if (settings && numNights > settings.maxBookingLength) {
      errors.push(`Maximum booking length is ${settings.maxBookingLength} nights`);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const bookingData = {
        ...
        cabinPrice: selectedCabin!.discount > 0 ? selectedCabin!.price - selectedCabin!.discount : selectedCabin!.price,
        ...
        remainingAmount: priceBreakdown.totalPrice - (formData.depositPaid ? priceBreakdown.depositAmount : 0),
      };
      ...
      <Card>
        ...
        <CardBody className="space-y-4">
          ...
          <Select
            ...
            onSelectionChange={(keys) => handleInputChange("cabin", Array.from(keys)[0] as string)}
            isRequired
          >
            {(cabins || []).map((cabin) => (
              <SelectItem key={cabin._id}>
                {cabin.name} - {formatCurrency(cabin.price)}/night (Capacity: {cabin.capacity})
                {cabin.discount > 0 && ` - ${formatCurrency(cabin.discount)} discount`}
              </SelectItem>
            ))}
          </Select>
          ...
          <Select
            ...
            onSelectionChange={(keys) => handleInputChange("customer", Array.from(keys)[0] as string)}
            isRequired
          >
        ...
        <CardBody className="space-y-4">
          <div className="space-y-4">
            <Switch
              ...
              onValueChange={(checked) => handleInputChange("hasBreakfast", checked)}
            >
              Include Breakfast ({formatCurrency(settings?.breakfastPrice || 0)}/person/night)
            </Switch>
            ...
            <Switch
              ...
              onValueChange={(checked) => handleInputChange("hasEarlyCheckIn", checked)}
            >
            ...
            <Switch
              ...
              onValueChange={(checked) => handleInputChange("hasLateCheckOut", checked)}
            >
      ...
      <Card>
        ...
        <CardBody className="space-y-4">
          ...
          <Input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleInputChange("observations", e.target.value)}
          />
        </CardBody>
      </Card>
      ...
      <Card>
        ...
        <CardBody className="space-y-4">
          <Select
            ...
            onSelectionChange={(keys) => handleInputChange("paymentMethod", Array.from(keys)[0] as string)}
          >
          ...
          {settings?.requireDeposit && (
            <Switch
              ...
              onValueChange={(checked) => handleInputChange("depositPaid", checked)}
            >
            ...
            <div className="space-y-2">
              ...
              {settings?.requireDeposit && (
                <>
                  ...
                  <div className="flex justify-between text-sm">
                    <span>Remaining</span>
                    <span>{formatCurrency(priceBreakdown.totalPrice - priceBreakdown.depositAmount)}</span>
                  </div>
                </>
              )}
            </div>
