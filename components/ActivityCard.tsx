...
export default function ActivityCard({ status, name, stayDuration }: Props) {
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'unconfirmed': return 'text-warning-600';
      case 'checked-in': return 'text-primary-600';
      case 'checked-out': return 'text-default-600';
      case 'cancelled': return 'text-danger-600';
  ...
  return (
    <div className='bg-content1 rounded-lg border border-divider p-3'>
      ...
      <div className='hidden md:flex items-center gap-4'>
        ...
        <p className={`w-32 capitalize text-center ${getStatusColor(status)}`}>{status}</p>
