...
async function verifyFix() {
  try {
    ...
    console.log(`📊 Found ${cancelledBookings.length} cancelled bookings in database`);
    
    if (cancelledBookings.length === 0) {
      console.log('ℹ️  No cancelled bookings found. Creating a test scenario...');
      ...
      const calculatedTotal = validBookings.reduce((sum, b) => sum + (b.totalPrice || 0), 0);
      
      console.log(`🧮 Calculated total (excluding cancelled): $${calculatedTotal}`);
      console.log(`💾 Stored total: $${customer.totalSpent}`);
      
      if (Math.abs(calculatedTotal - customer.totalSpent) < 0.01) {
        console.log('✅ VERIFICATION PASSED: Customer total spent correctly excludes cancelled bookings');
      } else {
        console.log('❌ VERIFICATION FAILED: Mismatch between calculated and stored totals');
      }
    } else {
      // There are cancelled bookings, verify they're not included
      const customersWithCancelled = Array.from(new Set(cancelledBookings.map(b => b.customer)));
      console.log(`👥 ${customersWithCancelled.length} customers have cancelled bookings`);
      
      for (const customerId of customersWithCancelled.slice(0, 3)) { // Check first 3
        ...
        const cancelledForCustomer = allBookings.filter(b => b.status === 'cancelled');
        
        const calculatedTotal = validBookings.reduce((sum, b) => sum + (b.totalPrice || 0), 0);
        const cancelledTotal = cancelledForCustomer.reduce((sum, b) => sum + (b.totalPrice || 0), 0);
        
        console.log(`\n👤 Customer ${customerId}:`);
        console.log(`   📊 Total bookings: ${allBookings.length} (${cancelledForCustomer.length} cancelled)`);
        ...
        if (Math.abs(calculatedTotal - customer.totalSpent) < 0.01) {
          console.log(`   ✅ CORRECT: Customer total excludes cancelled bookings`);
        } else {
          console.log(`   ❌ ERROR: Customer total includes cancelled bookings`);
        }
      }
    }
