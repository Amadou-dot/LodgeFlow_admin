...
export function useURLFilters<T extends Record<string, any>>({
  ...
  const buildURL = useCallback((updates: Partial<T>) => {
    const params = new URLSearchParams(searchParams.toString());
    
    Object.entries(updates).forEach(([key, value]) => {
      if (value === undefined || value === null || value === '' || 
          (filterConfig[key]?.defaultValue !== undefined && value === filterConfig[key].defaultValue)) {
        params.delete(key);
      } else {
  ...
  const updateFilter = useCallback((key: keyof T, value: any, addToHistory: boolean = false) => {
  ...
  const updateFilters = useCallback((updates: Partial<T>, addToHistory: boolean = false) => {
    const newURL = buildURL(updates);
  ...
  const clearFilter = useCallback((key: keyof T) => {
    const updates = { [key]: filterConfig[key as string]?.defaultValue } as Partial<T>;
    const newURL = buildURL(updates);
    router.replace(newURL, { scroll: false });
  }, [router, buildURL, filterConfig]);
...
export const bookingsFilterConfig: URLFilterConfig = {
  ...
  status: { 
    ...
    validValues: ['all', 'unconfirmed', 'confirmed', 'checked-in', 'checked-out', 'cancelled']
  },
  search: { type: 'string', defaultValue: undefined },
  sortBy: { 
    type: 'string', 
    defaultValue: 'checkInDate',
    validValues: ['created_at', 'checkInDate', 'totalPrice', 'guestName', 'cabinName']
  },
  sortOrder: { 
    type: 'string', 
    defaultValue: 'desc',
